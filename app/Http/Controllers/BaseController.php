...
class BaseController extends Controller
{
...

	{
		...
		$ads = Advertise::with('ads_type')
			->where('end_date', '>=', Carbon::now());
		$top_ads = $ads->where('advertise_type_id', 1)->get();
		if (!$top_ads->isEmpty()) {
			$top_ads = $top_ads->random(1);
		}
		$top_right_ads = $ads->where('advertise_type_id', 3)->get();
		if (!$top_right_ads->isEmpty()) {
			$top_right_ads = $top_right_ads->random(2);
		}
		$home_top_news_slider = $ads->where('advertise_type_id', 10)->get();
		$main_right_ads = $ads->where('advertise_type_id', 11)->get();
